...
function drawAssemblyMap(selector, filldata, mapdata, settings) {
    ...
    filldata_won = filldata.filter(function (filldatapoint) {
        // console.log("filldatapoint", filldatapoint);
        return filldatapoint.status === "Won"
    })

    console.log("filldatapoint", filldata_won);
    ...
    constituency.transition().duration(400)
        .attr('fill', function (d) {
            var fdFillData = filldata_won.filter(function (obj) {
                // console.log(obj)
    ...
    d3.select("#seats-of-party").style("color", partycolors[settings.year][settings.party])
        .html(filldata_won.length + "/" + mapdata.length)
